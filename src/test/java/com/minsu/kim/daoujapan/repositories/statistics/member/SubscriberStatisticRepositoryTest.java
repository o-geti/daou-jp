 ...
 */
@SpringBootTest
class SubscriberStatisticRepositoryTest {
  @Autowired private SubscriberStatisticRepository repository;

  @Test
  @DisplayName("검색 날짜에 맞추어 가입자 수 데이터가 잘 나오는지 확인한다.")
  void findByRecordTimeBetween() {
    ...
    // data.sql의 subscriber_statistics insert 문 가장 첫번째 값
    assertThat(result.getContent().getFirst().getSubscriberCount()).isEqualTo(983);
  }
  ...
  void findByDeleteDtIsNullAndId() {
    ...
    //    INSERT INTO subscriber_statistics (record_time, subscriber_count, delete_dt) VALUES
    // ('2024-10-28 00:00:00', 983, null);
